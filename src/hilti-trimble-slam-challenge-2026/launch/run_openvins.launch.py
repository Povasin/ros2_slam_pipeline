...
launch_args = [
    ...
    DeclareLaunchArgument(
        name="config",
        default_value="hilti_openvins",
        description="config folder name: hilti_openvins",
    ),
    DeclareLaunchArgument(
        name="config_path",
        default_value="",
...
def launch_setup(context):
    ...
    if not config_path:
        configs_dir = os.path.join(get_package_share_directory("challenge_tools_ros"), "config")
    ...
    node2 = Node(
        ...
        arguments=[
            "-d"
            + os.path.join(
                get_package_share_directory("challenge_tools_ros"), "config", "rviz/rviz_ov.rviz"
            ),
            "--ros-args",
            "--log-level",
            "warn",
    ...
    node4 = Node(
        ...
        output="screen",
        parameters=[
            {"topic": "/ov_msckf/odomimu"}, 
            {"file_path": "/home/kirill_fdx/ros2_ws/data/trajectory.txt"} 
        ],
    )
    node5 = Node(
            package='challenge_tools_ros',
